...
public class SetBeveragePriceLabel extends JLabel{
	
	public SetBeveragePriceLabel(BeverageInfo[] beverageInfo, int num) {
		setLocation(locationX(num), locationY(num));
		setSize(100, 15);
		int originPrice = beverageInfo[0].getPrice();
		int beveragePrice = beverageInfo[num].getPrice();
		beveragePrice = beveragePrice - originPrice;
		if(beveragePrice >= 0) {
			setText("+" + String.valueOf(beveragePrice));
...
//		       setFont(customFont);
	...
	public static int locationY(int num) {
		if (num / 4 == 0) {
			return (num / 4) * 100 + 135;
		} else {
			return (num / 4) * 115 + 145;
		}
	}
}
