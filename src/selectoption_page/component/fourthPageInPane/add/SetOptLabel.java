...
import selectoption_page.component.fourthPageInPane.add.menuText.SetBeveragePriceLabel;
import selectoption_page.component.fourthPageInPane.add.menuText.SetSidePriceLabel;
import selectoption_page.component.fourthPageInPane.add.menuText.SideNameLabel;

public class SetOptLabel extends JLabel {
	SideInfo[] sideInfo;
	
	SideButton[] sideButton;
	SideNameLabel[] sideNameLabel;
	SetSidePriceLabel[] sidePriceLabel;
	List<SideButton> sideBtns = new ArrayList<>();
	List<SideNameLabel> sideNLabels = new ArrayList<>();
	List<SetSidePriceLabel> sidePLabels = new ArrayList<>();
	{
		sideInfo = SideDAO.getSideInfo();
		sideButton = new SideButton[sideInfo.length];
		sideNameLabel = new SideNameLabel[sideInfo.length];
		sidePriceLabel = new SetSidePriceLabel[sideInfo.length];
		for (int i = 0; i < sideInfo.length; ++i) {
			sideButton[i] = new SideButton(sideInfo, i);
			sideNameLabel[i] = new SideNameLabel(sideInfo, i);
			sidePriceLabel[i] = new SetSidePriceLabel(sideInfo, i);
			sideButton[0].getCheck().setVisible(true);
	...
	JLabel sideLabel = new JLabel() {
		{
			...
			for (JLabel sideNLabel : sideNLabels) {
				add(sideNLabel);
			}
	...
	List<SetBeveragePriceLabel> beveragePLabels = new ArrayList<>();
	{
		...
		beveragePriceLabel = new SetBeveragePriceLabel[beverageInfo.length];
		for (int i = 0; i < beverageInfo.length; ++i) {
			...
			beveragePriceLabel[i] = new SetBeveragePriceLabel(beverageInfo, i);
			beverageButton[0].getCheck().setVisible(true);
